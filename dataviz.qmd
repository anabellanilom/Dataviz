...
## Datos sobre representación de las mujeres en los gabinetes del mundo entre 1966 y 2023
...
patchwork + plot_annotation(
  title = 'Evolución en el tiempo de la representación de las mujeres en los gabinetes de América Latina')
...
patchwork +
  plot_annotation(
    title = paste0(
      ...
      "Evolución en el tiempo de la representación de las mujeres",
      "</span><br>",
      "<span style='font-size:18pt; font-weight:700;'>",
      "en los gabinetes de América Latina",
      "</span>"
    ),
...
## Plot Boxplot décadas en todo el mundo 
p3<- base_paMundo %>%
  filter(year > 1980) %>%
  mutate(decada = paste0(floor(year / 10) * 10, "s")) %>%
  ggplot(aes(x = decada, y = proporcion, fill = decada)) +   # ✅ CAMBIO 1
...
patchwork<- p3+p1
patchwork + plot_annotation(
  title = 'Comparación evolución en el tiempo de la representación de las mujeres en los gabinetes del mundo y de América Latina')

```

## Comparación base de WhoGov y base de datos de elaboración propia
...
# Agrupar por año y calcular proporción promedio
df <- base_min %>%
  group_by(anio) %>%
  summarise(
...
# Agrupar por país y año, y calcular proporción promedio
df <- base_min %>%
  group_by(pais, anio) %>%
  summarise(
    proporcion = mean(muj_gab, na.rm = TRUE),
...
# Crear gráfico interactivo con facet_wrap por país
p2 <- ggplot(df, aes(x = anio, y = proporcion)) +
  geom_line() +
  ggiraph::geom_point_interactive(
    aes(
  ...
  scale_y_continuous(labels = percent_format(accuracy = 1)) +
  labs(
    title = "Porcentaje de ministras en los gabinetes de América Latina",
    subtitle = "2000–2025",
    x = "Año", y = "Porcentaje de ministras"
  ) +
  theme_minimal() +
...
# Crear gráfico interactivo con línea en 40%
p2 <- ggplot(df, aes(x = anio, y = proporcion)) +
  geom_line() +
  ggiraph::geom_point_interactive(
    aes(
  ...
  geom_hline(yintercept = 0.45, linetype = "dashed", color = "red", linewidth = 0.5) +  # 🔴 Línea del 40%
  facet_wrap(~ pais) +
  scale_y_continuous(labels = percent_format(accuracy = 1), limits = c(0, 1)) +
  labs(
    title = "Porcentaje de ministras en los gabinetes de América Latina",
    subtitle = "2000–2025. Línea roja indica el umbral del 40%",
    x = "Año", y = "Porcentaje de ministras"
  ) +
  theme_minimal() +
  theme(
